...
export async function calendarIcs(userId: string) {
  const tasks = await prisma.academicTask.findMany({ where: { userId, removedAt: null }, include: { course: true, recommendations: { where: { userId }, orderBy: { updatedAt: "desc" }, take: 1 } }, orderBy: { dueAt: "asc" } });
